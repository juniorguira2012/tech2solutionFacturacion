...
const Clientes = () => {
  ...
  const exportarAExcel = () => {
    ...
    const filas = clientes.map(c => {
      // Helper para escapar comillas dobles y envolver el texto de manera segura
      const mapearCampo = (valor) => {
        const texto = String(valor ?? '').replace(/"/g, '""'); // Escapa comillas internas
        return `"${texto}"`; // Encapitula el contenido
      };

      return [
        mapearCampo(c.nombre),
        mapearCampo(c.rnc),
        mapearCampo(c.telefono),
        mapearCampo(c.email),
        mapearCampo(c.direccion),
        mapearCampo(c.zona),
        mapearCampo(c.categoria || 'Bronce')
      ].join(",");
    });

    // 1. 🚨 EL TRUCO PARA EXCEL: Anteponemos 'sep=,\n' para forzar las columnas bien tabuladas
    const csvContent = "sep=,\n" + encabezados.join(",") + "\n" + filas.join("\n");

    try {
      // 2. Usamos el Blob con el BOM UTF-8 (\uFEFF) para proteger tildes, Ñs y caracteres especiales
      const blob = new Blob(['\uFEFF' + csvContent], { type: 'text/csv;charset=utf-8;' });
      const url = URL.createObjectURL(blob);

      // 3. Creamos el link de descarga virtual sin límites de tamaño por URL
      const link = document.createElement("a");
      link.href = url;
      
      const fechaFormateada = new Date().toLocaleDateString().replace(/\//g, '-');
      link.setAttribute("download", `Clientes_OneRedRD_${fechaFormateada}.csv`);
      
      // 4. Ejecutamos la descarga y limpiamos la memoria caché
      document.body.appendChild(link);
      link.click();
      document.body.removeChild(link);
      URL.revokeObjectURL(url);

      mostrarToast?.("Lista de clientes exportada con éxito", "success");
    } catch (error) {
      console.error("Error al exportar clientes:", error);
      mostrarToast?.("No se pudo generar la exportación", "error");
    }
  };
      ...
      onConfirm: async () => {
        try {
          ...
          if (exito) {
            ...
            // 🚨 TIP EXTRA DE FRONTEND: Si tu contexto no limpia el estado local de clientes automáticamente,
            // puedes mandar a llamar a la función que recarga los clientes aquí para limpiar la pantalla de una vez:
            // if (typeof recargarClientes === 'function') recargarClientes();
          }
        } catch (error) {
