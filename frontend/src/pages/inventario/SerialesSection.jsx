...
const serialesFiltrados = useMemo(() => {
  ...
  if (filtroTecnicoId) {
    resultados = resultados.filter(s => String(s.technicianId) === String(filtroTecnicoId));
  }
  ...
  return resultados.filter(s =>
    ...
    s.producto?.nombre?.toLowerCase().includes(busquedaLower) || // 👈 Agregado ?. en nombre
  ...
  {/* 1. Si no hay seriales aún pero el arreglo se está poblando */}
      {seriales.length === 0 ? (
        <div className="flex flex-col items-center justify-center p-12 text-slate-400">
          <p className="text-xs font-bold uppercase tracking-wider">Cargando seriales...</p>
        </div>
      ) : paginatedSeriales.length > 0 ? (
        /* 2. Si hay seriales paginados para mostrar */
          ...
          <tbody className="divide-y text-[11px]">
            {paginatedSeriales.map((serial) => (
              <tr key={serial.id} className="hover:bg-slate-50 transition-colors">
                <td 
                  className="px-6 py-3 font-black text-brand uppercase cursor-pointer hover:underline" 
                  onClick={() => abrirModalHistorial(serial)}
                >
                  {serial.serialNumber}
                </td>
                <td className="px-6 py-3 font-bold text-slate-800 uppercase">
                  {serial.producto?.nombre || 'N/A'}
                </td>
    ...
    /* 3. Si se filtró pero no hubo coincidencias */
    <div className="flex flex-col items-center justify-center p-12 text-slate-400">
