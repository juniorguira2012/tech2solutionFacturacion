...
export class CategoriesService {
  ...
  async remove(id: number): Promise<{ message: string }> {
    // 1. Primero, buscamos la categoría que se quiere eliminar para obtener su nombre.
    const categoriaAEliminar = await this.categoryRepository.findOneBy({ id });
    if (!categoriaAEliminar) {
      throw new NotFoundException(`Categoría con ID ${id} no encontrada`);
    }

    // 2. Contamos cuántos productos están usando esa categoría.
    const productosEnUso = await this.productRepository.count({
      where: { categoria: categoriaAEliminar.nombre },
    });

    // 3. Si hay uno o más productos, lanzamos un error de conflicto.
    if (productosEnUso > 0) {
    ...
    // 4. Si no hay productos en uso, procedemos con la eliminación.
